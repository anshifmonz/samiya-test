...
import {
  ...
  formatPaymentError,
} from "utils/payment/errorHandling";
...
export async function initiatePaymentSession(
  ...
  try {
    ...
    if (orderError || !order)
      return { body: { error: "Order not found" }, status: 404 };
    ...
    if (new Date(processingCheckout.expires_at + "Z").getTime() < Date.now()) {
      ...
      return {
        body: {
          error: "Checkout reservation has expired. Please re-checkout.",
        },
        status: 400,
      };
    }
    ...
    const cfOrderId = generateOrderId("cf");

    // Prepare Cashfree order data aligned with 2025-01-01
