  ...
  try {
    ...
    if (payment.status !== "completed") {
      if (!payment.cf_order_id)
        return { error: "Payment not linked to gateway order", status: 400 };

      const cf = await fetchCashfreeOrder(payment.cf_order_id);
      if (!cf.success)
        return { error: "Failed to verify payment status", status: 400 };
