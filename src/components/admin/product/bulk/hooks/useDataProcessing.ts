...
export const useDataProcessing = (
  ...
  const findCategoryId = useCallback(
    (categoryInput: string): { id: string | null; matched: boolean } => {
  ...
  const parseImageData = useCallback(
    (
      imageString: string
    ): { data: Record<string, { hex: string; images: string[] }>; errors: string[] } => {
      ...
      try {
        ...
        for (const colorGroup of colorGroups) {
          ...
          if (pipeIndex === -1) {
            errors.push(
              `Invalid image format: "${trimmedGroup}". Expected format: colorName:#hexCode|url1,url2`
            );
            continue;
          }
          ...
          if (colonIndex === -1) {
            errors.push(
              `Invalid color format: "${colorInfo}". Expected format: colorName:#hexCode`
            );
            continue;
          }
          ...
          if (validUrls.length > 0) {
            ...
            if (result[colorName]) {
              ...
              if (result[colorName].hex !== hexCode.toUpperCase()) {
                errors.push(
                  `Color "${colorName}" already defined with hex code ${
                    result[colorName].hex
                  }. Skipping duplicate with hex code ${hexCode.toUpperCase()}`
                );
                continue;
              }
        ...
        errors.push(
          `Error parsing image data: ${error instanceof Error ? error.message : 'Unknown error'}`
        );
      }
    ...
    (data: string): ParsedProduct[] => {
      ...
      for (let i = 0; i < dataLines.length; i++) {
        ...
        headers.forEach((header, index) => {
          ...
          const headerLower = hasCustomHeaders
            ? header.toLowerCase()
                ...
                if (categoryResult.matched && categoryResult.id) {
                  ...
                  const suggestions =
                    availablePaths.length > 0
                      ? `Available category paths include: ${availablePaths
                          .slice(0, 3)
                          .join(', ')}...`
                      : 'Please check the category name or path.';

                  product.errors.push(`Category "${value}" not found. ${suggestions}`);
                }
              }
              break;
            case 'tags':
  ...
  }, [persistentData, isInitialized, expectedHeaders, parsePastedData]);
