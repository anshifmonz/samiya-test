...
const Category: FC<{ category: CategoryWithProducts }> = ({ category }) => {
  const [products, setProducts] = useState<SectionProduct[]>(category.products);
  const [offset, setOffset] = useState<number>(category.products.length);
  ...
  const fetchMoreProducts = async (): Promise<void> => {
    ...
    try {
      const { data } = await apiRequest(
        `/api/categories/${category.id}?limit=12&offset=${offset}`,
        {
          showLoadingBar: true
        }
      );
